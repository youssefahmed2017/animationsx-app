...
Magic-link (OTP) email auth via Supabase (`src/app/login/page.tsx`, `supabase.auth.signInWithOtp`). Three separate Supabase client constructors exist, each for a distinct execution context — use the matching one, don't mix them up:
...
- `src/lib/supabase/middleware.ts` — used from `src/proxy.ts` to refresh the session cookie on every request so server components see current auth state.

### Database schema

`supabase/schema.sql` is the source of truth (run manually in the Supabase SQL editor or `supabase db push` — not an automated migration pipeline). Two tables: `profiles` (auto-created via an `on_auth_user_created` trigger on `auth.users`) and `animations` (with `forked_from_id` self-reference for the fork/remix feature, and `view_count`/`fork_count` maintained by SQL RPCs `increment_view_count`/`increment_fork_count`). RLS is enabled on both tables with public read policies; there are intentionally no client-facing insert/update policies on `animations` since all writes go through the service-role key in the API route.
